...
export default function LeaguesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      leaguesApi.create(newLeague.name, newLeague.description || undefined),
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-[#D9D4CC] rounded-lg p-14 flex flex-col items-center gap-5 text-center">
          ...
          <div>
            <p className="font-heading font-black text-[#111111] mb-1">
              {t("leagues.empty.title")}
            </p>
            <p className="text-[#666666] text-sm">
              {t("leagues.empty.subtitle")}
            </p>
          </div>
      ...
      <Modal
        ...
        title={t("leagues.createModal.title")}
        size="lg"
      >
        <form
          onSubmit={(e) => {
        ...
        >
          <Input
            ...
            onChange={(e) =>
              setNewLeague((p) => ({ ...p, name: e.target.value }))
            }
            required
            minLength={3}
            maxLength={100}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-[#666666] uppercase tracking-[0.08em] font-sans">
              ...
              <span className="text-[#999390] normal-case tracking-normal font-normal">
                {t("leagues.field.optional")}
              </span>
            </label>
            <textarea
              value={newLeague.description}
              onChange={(e) =>
                setNewLeague((p) => ({ ...p, description: e.target.value }))
              }
              placeholder={t("leagues.field.description")}
              maxLength={500}
              rows={2}
              className="w-full rounded px-3 py-2 text-sm text-[#111111] bg-white border border-[#D9D4CC] hover:border-[#B8B2AA] focus:border-[#111111] focus:outline-none placeholder:text-[#999390] resize-none transition-[border-color] duration-150"
            />
          </div>
      ...
      <Modal
        ...
        title={t("leagues.joinModal.title")}
        size="md"
      >
        <form
          onSubmit={(e) => {
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              required
              maxLength={20}
              className="w-full rounded px-4 py-3 text-base text-[#111111] bg-white border border-[#D9D4CC] hover:border-[#B8B2AA] focus:border-[#111111] focus:outline-none placeholder:text-[#D9D4CC] uppercase tracking-widest text-center font-mono transition-[border-color] duration-150"
            />
          </div>
