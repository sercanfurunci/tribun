...
export default function LeaguesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: ({ data }) => {
      toast.success(`"${data.league.name}" created!`);
  ...
  return (
    <div className="space-y-8 animate-fade-up">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-widest mb-1 font-heading">Prediction</p>
      ...
      {isLoading ? (
        ...
        >
          ...
          <div>
            <p className="font-heading font-semibold text-white mb-1">No leagues yet</p>
            <p className="text-slate-600 text-sm">Create a league or join with an invite code</p>
          </div>
